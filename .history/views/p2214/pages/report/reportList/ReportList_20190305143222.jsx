...
//批量隐藏功能
             ...
             .then((res) => {
                  if(res.code==0){
                      ...
                      for(var i=0;i<cardList.length;i++){
                          ...
                          this.getreportList(cardList[i].patientId);
                      }
                      this.setState({
                        patList:cardList
                      })
    ...
    getreportList(patientId = '') {
         ...
         Api
             ...
             .then((res) => {
              ...
              console.log("rr1",this.state.reportList)
  ...
  getUser() { // 获取实名制
        ...
        .then((res) => {
            ...
            if (res.code == 0) {
                 ...
                 this.setState({
                    ...
                    visitNo:visitNo,
                    showIOS4:true,
                    msg:'您将向'+this.state.doctor.name+'医生进行免费报告解读，此次报告解读可追加5个问题，若医生解读报告，有效期为48小时。若医生未解读您的报告，本次报告解读不限有效期。',
    ...
    inquiry(type,hisDoctorName,visitNo){
        ...
        if(type==4){
            this.setState({
                showIOS1:true,
                msg:'你的报告已被绑定此就诊人的其他账户申请解读，你无法再次申请解读',
  ...
  render() {
    const {patList,currentName,reportList,currentId,isPatShow,currentPatient,msg}=this.state
            ...
            {patList&&patList.map((item,index)=>{
                return(
                    <p
                        ...
                        className={`${item.active?'active':''}`} style={{fontWeight:'bold'}}>
                        {item.patientName}
        ...
        {reportList[currentId]&&reportList[currentId].map((item,index)=>{
            return(
                ...
                <div className='reportPerson'>
                    ...
                    <p className='time'>
                    <span style={{fontSize:'12px',fontWeight:'normal'}}>就诊人：{currentName}</span> <span style={{fontSize:'14px',fontWeight:'bold'}}>{item.report[0].type=='1'?item.report[0].Requested_date_time:item.report[0].Req_date_time}</span></p>
                ...
                <div className='reportPerson'>
                    ...
                    <p className='time'><span style={{fontSize:'12px',fontWeight:'normal'}}>医生：{item.doctorRealName}</span> <span style={{fontSize:'14px',fontWeight:'bold'}}>{item.report[0].type=='1'?item.report[0].Requested_date_time:item.report[0].Req_date_time}</span></p>
                    {item.inquiryId=='0'&&item.has==1&&<span className='yes' onClick={()=>{
                          //已出未解读
                    ...
                    </div>
                {
                    item.report&&item.report.map((item1,index1)=>{
                        return(
                            ...
                            <span style={{fontSize:'12px',color:'#999'}}>检查项：</span> <span style={{fontSize:'14px',color:'#333'}}>{item1.type=='1'?item1.Sheet_title+'  ':item1.Exam_item+'  '  }   </span>   
