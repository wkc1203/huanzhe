            ...
            return(
                ...
                <div className='reportPerson'>
                  ...
                  {item.inquiryId=='0'&&item.getTime=='1'&&item.hasTimeOut=='1'&&<span className='no' onClick={()=>{
                    ...
                    {item.inquiryId.length>2&&item.myself=='1'&&(!!item.inquiry&&(item.inquiry.status!=='4'&&item.inquiry.status=='5'))&&<span className='already' onClick={()=>{
                          //已解读是自己
                          this.inquiry(2,item.hisDoctorName,item.inquiryId)
                    }}>查看解读详情</span>}
                    {item.inquiryId.length>2&&(item.myself=='0'||(!!item.inquiry&&(item.inquiry.status=='4'||item.inquiry.status=='5')))&&<span className='no' onClick={()=>{
                          //已解读不是自己
                          this.setState({
                            timeout:false,
                        })
