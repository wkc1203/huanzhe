...
//批量隐藏功能
             ...
             .then((res) => {
                  if(res.code==0){
                      ...
                      for(var i=0;i<cardList.length;i++){
                          ...
                          console.log("S",cardList[i].patientName);
                      ...
                      console.log('pp',this.state.patList)
                        ...
                        for(var i=0;i<report.length;i++){
                          ...
                          var j; 
                                for(j=0;j<report[i].report.length;j++){
                                     if(j==0){
                                        var vDate=Math.round(new Date(report[i].report[j].Visit_date).getTime()/1000);
                                        var nDate=Math.round(new Date().getTime()/1000);
                                        console.log("date",vDate,nDate) ;
                                        if(vDate<=nDate){
                                            report[i].getTime=1;
                                        }else{
                                            report[i].getTime=2;
  ...
  render() {
    const {patList,currentName,reportList,currentId,isShowProtocol,currentPatient,msg,timeout}=this.state
        ...
        {reportList&&reportList.length>0&&reportList.map((item,index)=>{
            return(
                ...
                <div className='reportPerson'>
                    ...
                    {(item.inquiryId=='0'&&item.getTime!=='1'&&item.allOnLineReportResult==1&&item.reportInterpretateFlag=='1')||(item.inquiryId=='0'&&item.hasTimeOut!=='1'&&item.getTime=='1'&&item.allOnLineReportResult==1&&item.reportInterpretateFlag=='1')&&<span className='yes' onClick={()=>{
                        //已出未解读
                  ...
                  {item.inquiryId=='0'&&item.getTime=='1'&&item.hasTimeOut=='1'&&<span className='already' onClick={()=>{
                    this.setState({
                        showIOS1:true,
                        timeout:true,
                    ...
                    {item.inquiryId.length>2&&item.myself=='0'&&<span className='already' onClick={()=>{
                          //已解读不是自己
                          this.setState({
                            timeout:false,
                        })
