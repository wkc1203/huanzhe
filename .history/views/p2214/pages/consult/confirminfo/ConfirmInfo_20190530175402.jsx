...
onChange = (files,file,index) => {
    ...
    for(var i=0;i<files.length;i++){
        ...
        var image = new Image();
                image.src =window.URL.createObjectURL(files[i]).substring(5,window.URL.createObjectURL(files[i]).length);
        ...
        var uuid=S4+S4+"-"+S4+"-"+S4+"-"+S4+"-"+S4+S4+S4;
        var filename=that.randomName()+uuid+'.png';
        console.log(window.URL.createObjectURL(files[i]).substring(5,window.URL.createObjectURL(files[i]).length))
        formData.append('key',filename);
        formData.append("policy",sign.policy);
        formData.append("callback",sign.callback);
        formData.append("signature",sign.signature);
        formData.append("OSSAccessKeyId",sign.OSSAccessKeyId); 
        formData.append('file',this.base64ToBlob(files[i].path)); 
        $.ajax({
            url: 'https://ihoss.oss-cn-beijing.aliyuncs.com', 
            method: 'POST',
            processData: false,
