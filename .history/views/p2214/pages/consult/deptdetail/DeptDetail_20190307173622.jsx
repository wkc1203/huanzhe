            ...
            .then((res) => {
                ...
                wx.ready(function () {   //需在用户可能点击分享按钮前就先调用
                    wx.onMenuShareAppMessage({
                        ...
                        success: function () {
                          // 用户点击了分享后执行的回调函数
                         that.props.location.query.resource=1;
                        }
                    });
                })
            }, (e) => {
        ...
        return (
            <div className="page-dept-detail container1">
                ...
                <div className='header2' id="head">
                             ...
                             /*  docInfo.inquirys && docInfo.inquirys.map((item, index)=> {
                                    return (
                                        <div key={index}
                                            ...
                                            {item.type == '1' && item.isOnDuty == '0' ? '离线' : ''}
                                            {item.isFull == '1' &&item.type == '1'  && item.isOnDuty == '1'? '已满' : ''}
                                        </div>
                                  ...
                                  <p className='advice' onClick={()=>{
                                    this.context.router.push({
                                        ...
                                        query:{type:1,deptName:docInfo.deptName,deptId:docInfo.deptId,doctorName:docInfo.name,doctorId:docInfo.doctorId}
                                    })
                                    }}>投诉建议</p>
                                  <img   className='adviceimg'
                                    ...
                                    {isFavorite &&<p className='advice1' >已收藏</p>}
                                    {!isFavorite &&<p className='advice1' >请收藏</p>}
                    ...
                    <div className='data'>
                    <div className='item'>
                    <div>{docInfo.serviceTimes}<a style={{fontSize:'12px'}}>人</a></div>
                        <div>服务</div>

                        
                    </div>
                    <div className='item'>
                    <div>{/* docInfo.favoriteRate */}98<a style={{fontSize:'12px'}}>%</a></div>
                        <div>好评率</div>
                       
                    </div>
                    <div className='item'>
                    <div>{docInfo.workingLife}<a style={{fontSize:'12px'}}>年</a></div>
                        <div>从业年限</div>
