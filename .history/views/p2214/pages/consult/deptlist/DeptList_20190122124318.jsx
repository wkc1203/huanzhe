...
class Widget extends Component {
    ...
    loadMoreDataFn() {
         if(this.state.searchValue!=''){
             if(this.state.inquiryPage<=this.state.maxPage){
                this.search(this.state.searchValue);
             }
           

         }else{
             console.log("page",this.state.searchPage,this.state.maxPage)
             if(this.state.searchPage<=this.state.maxPage){
                this.selectDept(this.props.location.query.deptName, this.props.location.query.deptId,this.state.searchPage)
        ...
        Api
            ...
            .then((res) => {
                if (res.code == 0 && res.data != null) {
                   ...
                   for(var i=0;i<res.data.doctors.length;i++){
                    searchList.push(res.data.searchList[i])
                   }
                    this.setState({
