...
class Widget extends Component {
    ...
    addQuiry(){
        ...
        const params = {
         ...
         purpose:'免费报告解读',
         purposeType:'9',
         hisDoctorName:this.state.docInfo.hisDoctorName,
         hospitalVisitNo:this.state.visitNo,
    ...
    getreportList() {
        ...
        var report;   
        Api
            .getreportList({patientId:this.state.selectPatientId,hisDoctorName:this.state.docInfo&&this.state.docInfo.hisDoctorName?this.state.docInfo.hisDoctorName:'',test:'1'})
            .then((res) => {
                this.hideLoading();
                if (res.code == 0) {
                    if(res.data.length>0){
                        this.setState({
                            ...
                            visitNo:res.data[0].report[0].Visit_no,
                            inDate:res.data[0].inDate,
                        })
                        var data=res.data;
                        for(var i=0;i<data.length;i++){
                            if(this.state.docInfo.hisDoctorName==data[i].hisDoctorName){
                                Api.getDoctor({hospitalUserName:this.state.docInfo.hisDoctorName})
                                .then((res) => {
                                    if (res.code == 0) {
                                          this.setState({
                                              reportInfo:data[i],
                                ...
                                Api.getDoctor({hospitalUserName:data[i].hisDoctorName})
                                .then((res) => {
                                    if (res.code == 0) {
                                          this.setState({
                                              reportInfo:data[i],
                        ...
                        if(this.state.otherDoctor){
                            this.setState({
                                msg:' 查询到您可向陈某某医生发起免费报告解读，是否立即前往',
                                showIOS1:true,
                            })
                        }
...
onChange = (files, type, index) => {
  ...
  onTabChange = (key) => {
    ...
    render() {
        const {report,codeUrl,cardShow,policy,msg,callback,OSSAccessKeyId,open1,docInfo,cardList,consultList,imgArr,leftBindNum,
