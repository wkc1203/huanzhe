...
var interval2='';//发送后到最底部
var uuList = [];//随机filename
var nameList=[];//uuid
var imgArr1 = [];//图片
...
class Widget extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            status: '',//咨询状态
            canStop:true,//是否停止语音
            showPlus: false,//是否显示更多金莲
            name: this.props.location.query.name,//患者信息
            inquiryId: '',/* 咨询id */
            userId: '',/* 用户id */
            uId: '',/* 用户id */
            patHisNo: '',/* 医院就诊人编号 */
            ...
            appraisalLabel: '',/* 选择的评价标准 */
            appraisal: '',/* 评价详情 */
            newScore: '',//用户评价的分数
            itemList: 0,/* 评价指标列表 */
            payBack:true,//是否退款
            isOk: false,//咨询条数是否使用完
            newText: '',/* 用户评价详情 */
            timeShow: '',/* 倒计时 */
            numEnd: 5,/* 剩余消息发送条数 */
            newItem: '',/* 评价指标字符串 */
    ...
    componentDidMount() {
        ...
        var interval = setInterval(() => this.getChat(1), 1000);
        this.setState({
              interval:interval
         })
    }
            ...
            if (res.code == 0) {
                ...
                if (res.code == 0) {
                    ...
                    //else{
                        ...
                        //var that=this;
                        //socket = io.connect(
                        //    "https://tih.cqkqinfo.com/?inquiryId="+that.props.location.query.inquiryId+"&userId="+that.state.userId
                        //  );
                            
                            // socket.on("connect", function() {  
                            //  console.log("https://tih.cqkqinfo.com/?inquiryId="+that.props.location.query.inquiryId+"&userId="+that.state.userId)   
                            // });
                            // socket.on("message_inquiry",function(data){
  ...
  /*结束咨询方法*/
   ...
   /*播放语音*/
    ...
    const {isEvaluate,files,orderId,list,msgText,isShow,isEnd,docInfo,userInfo,doctorid,deptid,showPlus,interval,
        name,match,userId,uId,patHisNo,score,txtNum,t1,t2,t3,t4,t5,timeShow,numEnd,
        canEnd,appraisal,newScore,itemList,payBack,isOk,newText ,newItem,
        newTime,doctorName,noDoctor,isIos,msg,endding,end,sign,signature,formData,policy,callback,OSSAccessKeyId,key,evaluateTime,isBtn,inputText,isPlay}=this.state
        return (
                                    ...
                                    <div key={index} className="content-item">
                                        ...
                                        {item.voiceTime > 0 && <div id="a" className='left slide'>
                                            ...
                                            {item.voiceTime && <div
                                                ...
                                                <span
                                                    className='dura'>{item.voiceTime}</span>
                                            </div>}
                                            {item.url && <div className='flex'></div>}
                                        </div>}
