                    ...
                    if (res.code == 0) {
                        ...
                        if (res.data.inquiry.status != 1 && res.data.inquiry.status != 0) {
                           ...
                           var socket;
                            var that=this;
                            socket = io.connect(
                                "https://tih.cqkqinfo.com/?inquiryId="+that.props.location.query.inquiryId+"&userId=user"+that.state.userId
                              );
                              socket.on("message_inquiry",function(data){
                                   var list=that.state.list;
                                   var current=data;
                                   var currentunixTimestamp =Utils.timestampFordate(current.createTime,'Y-m-d H:i:s',false);
                                    var updateunixTimestamp =Utils.timestampFordate(current.updateTime,'Y-m-d H:i:s',false);
