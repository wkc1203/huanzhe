             ...
             .then((res) => {
                 if (res.code == 0) {
                     ...
                     checkDetail.addInfo=JSON.parse(res.data.subscribeInfo);
                      ...
                      var checkItem=JSON.parse(res.data.checkItem);
          ...
          <div className='check-order'>
             ...
             >
              ...
              {!!checkDetail.checkItem&&checkDetail.checkItem.length>0&&checkDetail.checkItem.map((item,index)=>{
                   return(
                    <div className="order-item righttab" key={index}
                    onClick={()=>{ 
                        
                        this.context.router.push({
                            pathname:'/ordermng/checkorder?check='+JSON.stringify(item)+"&case="+JSON.stringify(!!this.state.checkDetail.onlineCheckListCaseVo?this.state.checkDetail.onlineCheckListCaseVo:'')
                        }) 
                    }}
                    >
                        {item.type=='inspect'&&<p>{item.type=='inspect'?item.Lab_sheet_item:item.subItem}</p>}
                        {item.type=='check'&&<p>{item.Exam_class?item.Exam_class:''}{item.Exam_sub_class?"|"+item.Exam_sub_class:''}{item.Exam_lower_sub_class&&item.Exam_lower_sub_class!='ALL'?"|"+item.Exam_lower_sub_class:''}</p>}
                        <p>￥{item.Total_charges}</p>
                    </div>
                   )
          ...
          {!!checkDetail&&checkDetail.status=='4'&&<div className='order-info'>
               <div className='title'>
                ...
                <div className='info'>
                   ...
                   <div className='item'>
                      <p>支付流水号</p>
                      <p>{checkDetail.paySerialNumber}</p>
                   </div>
                   <div className='item'>
                      <p>支付时间</p>
                   ...
                   <div className='item'>
                      <p>退款流水号</p>
                      <p>{checkDetail.refundSerialNo}</p>
                   </div>
                   <div className='item'>
                      <p>退款时间</p>
