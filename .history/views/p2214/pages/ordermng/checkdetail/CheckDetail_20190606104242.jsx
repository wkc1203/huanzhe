  ...
  render() {
    ...
    return (
        <div className="container page-check-detail">
          ...
          <div className='person-info'>
               <div >
                  ...
                  <p style={{textAlign:'right'}}>年龄：{!!checkDetail.onlineCheckListCaseVo&&checkDetail.onlineCheckListCaseVo.patientAge?checkDetail.onlineCheckListCaseVo.patientAge:''}</p>
               </div>
               <div>
          ...
          <div className='check-order'>
              ...
              {!!checkDetail.checkItem&&checkDetail.checkItem.length>0&&checkDetail.checkItem.map((item,index)=>{
                   return(
                    <div className={`order-item ${!!item&&item.type=='treat'?'':'righttab'} ${!!item.item&&item.refund=='1'&&item.type!='treat'?'grey':!!item.refund&&item.refund=='1'?'grey':''}`} key={index}
                    onClick={()=>{ 
                       if(item.type!='treat'){
                        this.context.router.push({
                        ...
                        {item.type=='treat'&&<p className={`${!!item.item&&item.refund=='1'?'grey':!!item.refund&&item.refund=='1'?'grey':''}`}>{item.type=='treat'?item.Title:item.Title}</p>}
          ...
          {!!checkDetail&&checkDetail.status=='4'&&
        ...
        {!!checkDetail&&(checkDetail.status=='7')&& <p className='cancel'>已退款</p> }
              {!!checkDetail&&(checkDetail.status=='2'||checkDetail.status=='3')&& <p className='cancel'>已取消</p> }
              ...
              {!!checkDetail&&(checkDetail.status=='4')&& <p className='infos'>在线检验检查费用目前暂不支持线上退费，如需退费请到医院缴费窗口咨询。</p> }
